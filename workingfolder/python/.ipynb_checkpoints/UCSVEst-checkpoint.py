...
class UCSVEst:
    ...
    def draw_eps_eta(self,
        ...
        #print(covtau_y)
        chol_covtau_y = np.linalg.cholesky(xx2)
    ...
    def draw_var(self,
        ...
        f1 = np.exp((-0.5)*(((edraw-r_m1)/r_sig)**2)  )
        f2 = np.exp((-0.5)*(((edraw-r_m2)/r_sig)**2)  )
        fe = r_p*f1 + (1-r_p)*f2
        r_pt = (r_p*f1)/fe
...
est_fake.draw_eps_eta(var_eps_fake,var_eta_fake)
